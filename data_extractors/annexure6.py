...
HSN_LABELLED_RE = re.compile(
    r"(?:HSN|SAC|HSN/SAC|Service Accounting Code|SAC Code|Category of Service)\s*[:#-]?\s*(\d{4,8})",
    re.IGNORECASE,
)
...
BLOCK_END_PATTERN = re.compile(
    r"|".join([f"(?:{k})" for k in BLOCK_END_TOKENS]), re.IGNORECASE
)
...
def add_paths_to_env():
    if POPPLER_PATH and os.path.isdir(POPPLER_PATH):
        ...
        logging.info(f"Added Poppler path: {POPPLER_PATH}")
    else:
        logging.warning("Poppler path is invalid or not set.")

    if TESSERACT_CMD and os.path.exists(TESSERACT_CMD):
        pytesseract.pytesseract.tesseract_cmd = TESSERACT_CMD
        logging.info(f"Set Tesseract command: {TESSERACT_CMD}")
    else:
        logging.warning("Tesseract command is invalid or not set.")
...
def preprocess_image(img: Image.Image) -> Image.Image:
    try:
        ...
        img = Image.eval(img, lambda x: 0 if x < 170 else 255)
        return img
...
def safe_search(
    pattern: str, text: str, flags=re.IGNORECASE | re.DOTALL
) -> Optional[str]:
    m = re.search(pattern, text, flags)
    return m.group(1).strip() if m else None


# Improved regex matching with fallback
def find_first_match(patterns: list[str], text: str) -> str:
    for pat in patterns:
        v = safe_search(pat, text)
        if v:
            return v
    logging.warning(f"No match found for patterns: {patterns}")
    return ""
...
def extract_name_only(block: str, norm: str) -> Optional[str]:
    ...
    for ln in lines:
        # Skip junk: labels, invoice refs, totals, addresses, websites
        if re.search(
            r"(Invoice|No\.?|Number|Date|Order|Ref|ID|PAN|GSTIN|Total|Amount|Website|State|Layout|Road|Street|Floor|Pincode|Email|Phone)",
            ln,
            re.IGNORECASE,
        ):
            continue
        if re.match(
            r"^(From|To|Bill|Ship|Party|Customer|Buyer|Consignee|Invoice|Billed|Name:)",
            ln,
            re.IGNORECASE,
        ):
            continue
        ...
        # Reject lines that look like address fragments
        if len(ln.split()) <= 1 and not re.search(
            r"(Pvt|Ltd|LLP|Inc|Corp|Enterprises|Company|Associates|Systems|Services|Solutions|Technologies)",
            ln,
            re.IGNORECASE,
        ):
            continue

        # If looks like a proper company or has 2+ words, accept
        if len(ln.split()) >= 2 or re.search(
            r"(Pvt|Ltd|LLP|Inc|Corp|Enterprises|Company|Associates|Systems|Services|Solutions|Technologies|Healthcare|Communications)",
            ln,
            re.IGNORECASE,
        ):
            return ln
    ...
    if gstin_match:
        ...
        for ln in reversed(before):
            ...
            if (
                ...
                and not re.search(
                    r"(GSTIN|Invoice|No|Date|Total|Amount|Website)", ln, re.IGNORECASE
                )
            ):
                return ln
...
def parse_invoice_text(text: str) -> Dict[str, Optional[str]]:
    ...
    issued_scope = safe_search(
        r"(Issued\s*To.*?)(?:From|Seller|Supplier|Tax\s*Invoice|$)", norm
    )
    if issued_scope:
    ...
    from_scope = safe_search(
        r"(From.*?)(?:Issued\s*To|Bill\s*To|To\s*,|Tax\s*Invoice|$)", norm
    )
    if from_scope:
    ...
    if desc_match:
        ...
        for ln in raw_desc.splitlines():
            if re.search(
                r"\b(Qty|Quantity|Rate|Amount|Total|HSN|SAC)\b", ln, re.IGNORECASE
            ):
                break
    ...
    # --- HSN / SAC ---
    hsn_label = safe_search(
        r"(?:HSN|SAC|HSN/SAC|Service Accounting Code|SAC Code|Category of Service)[:\s]+(\d{4,8})",
        norm,
    )
    if not hsn_label:
    ...
    # --- Quantity ---
    qty = safe_search(
        r"(?:Quantity|No\. of (?:days|hours)|Units|Nos\.?|Qty)[:\s\-]*([0-9]+(?:\.[0-9]+)?)",
        norm,
    )
    data["Quantity"] = qty
    ...
    # --- Rate ---
    rate = safe_search(
        r"(?:Rate|Unit Price|Price|Value\s*per\s*piece)[:\s₹]*([0-9][0-9,\.]+)", norm
    )
    ...
    if not value:
        value = safe_search(
            r"(?:Grand\s*Total|Total|Net Value|Gross Value|Amount)[:\s₹]*([0-9][0-9,\.]+)",
            norm,
        )
    data["Value (Net of GST)"] = value

    # --- Calculate Value Per Piece ---
    if data["Value Per Piece"] == "N/A" and data["Quantity"] and data["Quantity"] != "N/A":
        try:
            data["Value Per Piece"] = str(
                float(data["Value (Net of GST)"].replace(",", ""))
                / float(data["Quantity"])
            )
        except ValueError:
            data["Value Per Piece"] = "Error"
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract invoice fields from mixed PDFs (text + scanned)"
    )
    parser.add_argument(
        "--in",
        dest="input_path",
        default="./fwdsamplesforannexvi"
        help="Input folder (or single PDF file)",
    )
    parser.add_argument(
        "--out",
        dest="output_xlsx",
        default="Extracted_Invoices.xlsx",
